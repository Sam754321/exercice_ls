...
static t_content *ex_list_content_new(struct stat *info, char *nam, long *flag)
{
        t_content *retlist;
        char *tmp_str;
        ...
        if (nam)
        {
                ...
                tmp_str = ft_strdup(ft_strrchr(nam, '/'));
                tmp_str ? retlist->file_name = tmp_str : (retlist->file_name = ft_strdup(retlist->path));
                ...
                retlist->stat_array_info = array_str_stat_info(info, flag);
        }
        return retlist;
}

static tt_list *dir_list_contentt_list(char *path, struct dirent *dir, long *flag)
{
        ...
        if (path && dir)
        {
                ...
                new_content = ex_list_content_new(&stat_file, content_path, flag);
...
static void new_or_push_headlst(tt_list **head, char *name_path, struct dirent *tab_dirent, long *flag)
{
        if (!(*head) && tab_dirent)
                *head = dir_list_contentt_list(name_path, tab_dirent, flag);
        else if (*head && tab_dirent)
                ex_list_push_back(head, dir_list_contentt_list(name_path, tab_dirent, flag));
}
...
tt_list *ex_get_dir_list(char *path, long *flag)
{
        ...
        if (path)
        {
                ...
                do
                {
                        ...
                        if (tab_dir_info)
                                new_or_push_headlst(head_lst, path, tab_dir_info, flag);
                } while (tab_dir_info);
                check_flag_up('U', flag) ? 0 : sort_list(*head_lst, flag);
                check_err(errno, "readdir");
                closedir(dir_opn);
